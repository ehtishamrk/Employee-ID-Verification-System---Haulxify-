...
el("loginForm").addEventListener("submit", async (event) => {
  ...
  // Without this, the button gave zero feedback while the sign-in request
  // was in flight — a slow network or a silently-blocked request (ad
  // blocker / firewall dropping googleapis.com) looked identical to "the
  // button does nothing," because nothing on screen changed either way.
  ...
  try {
    ...
    // On success, onAuthStateChanged (below) swaps loginView for appView.
  } catch (error) {
...
function loginErrorMessage(error) {
  switch (error.code) {
    ...
    case "auth/operation-not-allowed":
      return "Email/Password sign-in isn't turned on for this Firebase project yet (Authentication → Sign-in method).";
    case "auth/network-request-failed":
      return "Network error reaching Firebase — check your connection, or an ad blocker/firewall may be blocking googleapis.com.";
    default:
      return "Sign-in failed. Check your credentials and confirm this account is authorized.";
  }
}
...
onAuthStateChanged(auth, async (user) => {
  ...
  try {
    ...
    // Separate try: at this point the account IS authorized. A data-load
    // hiccup here (network blip, or the 18 pass docs failing to create on
    // a first-ever admin login) is a different problem and must not bounce
    // the user back to the login screen claiming they're unauthorized.
    try {
      await loadCards();
      ...
      console.error("Dashboard data failed to load:", error.code, error.message);
      alert(`Signed in, but some dashboard data failed to load: ${error.message || "unknown error"}\n\nTry refreshing the page.`);
    }
  } catch (error) {
    console.error("Sign-in blocked:", error.message);
    setHidden(el("loginError"), false);
    el("loginError").textContent = error.message === "role-lookup-failed"
      ? "Couldn't check this account's access — try again in a moment."
